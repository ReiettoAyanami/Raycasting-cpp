...
int main(void)
{
    ...
    std::shared_ptr<L::RayCaster> caster = std::make_shared<L::RayCaster>(L::RayCaster{Vector2{screenWidth / 2.f,screenHeight / 2.f}, M_PI_2, M_PI_2, 50, 1000.f, BLACK});
    ...
    {   
        ...
        if(GetMouseDelta().x != 0 || GetMouseDelta().y != 0) caster -> pointTo(GetMousePosition());
        //caster -> follow(GetMousePosition(), 50.f, deltaTime);
        
        caster -> move(KEY_W, KEY_S, KEY_A, KEY_D, Vector2{5.f, 5.f});
